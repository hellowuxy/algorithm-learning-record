...
public class LongestSubstring {
  ...
  /*
   * 解题思路1：使用两层循环遍历字符串str，i表示外层循环索引、j表示内层循环索引 (0≤i<j≤n)，不断地移动i、j，寻找最长的子串；查找过程如下：
  ...
  /*
   * 解题思路2：滑动窗口（使用HashSet实现），我们使用 HashSet 将字符存储在当前窗口 [i, j)[i,j)（最初 j = i）中。 然后我们向右侧滑动索引 j，如果它不在
   * HashSet 中，我们会继续滑动 j。直到 s[j] 已经存在于 HashSet 中。 此时，我们找到的没有重复字符的最长子字符串将会以索引 i 开头。如果我们对所有的 i
   * 这样做，就可以得到答案。
   *
